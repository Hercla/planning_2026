...
Public Sub Check_Presence_Simplifie()
    ...
    rowMatin = CLng(Module_Config.CfgValueOr("CHK_RowMatin", 10))
    rowAM = CLng(Module_Config.CfgValueOr("CHK_RowAM", 11))
    rowSoir = CLng(Module_Config.CfgValueOr("CHK_RowSoir", 12))
    rowNuit = CLng(Module_Config.CfgValueOr("CHK_RowNuit", 0)) ' 0 = pas de ligne nuit sur Jour
    headerRow = CLng(Module_Config.CfgValueOr("CHK_HeaderRow", 4))
    minInf = CLng(Module_Config.CfgValueOr("CHK_MinInfJour", 2))
...
Private Sub CheckPresenceForTeam(ByVal ws As Worksheet, ByVal teamName As String)
    ...
    ' Load config
    headerRow = CLng(Module_Config.CfgValueOr("CHK_HeaderRow", 4))
    minInfJour = CLng(Module_Config.CfgValueOr("CHK_MinInfJour", 2))
    minInfNuit = CLng(Module_Config.CfgValueOr("CHK_MinInfNuit", 1))
    infFunctions = Split(Module_Config.CfgTextOr("CHK_InfFunctions", "INF;IC"), ";")
    ...
    Set wsPers = ThisWorkbook.Sheets(Module_Config.CfgTextOr("SHEET_Personnel", "Personnel"))
    On Error GoTo CleanUp
...
Public Sub Debug_DayDetail()
    ...
    headerRow = CLng(Module_Config.CfgValueOr("CHK_HeaderRow", 4))
    firstRow = CLng(Module_Config.CfgValueOr("CHK_FirstPersonnelRow", 6))
...
Private Function LoadPersonnelDict() As Object
    Dim d As Object: Set d = CreateObject("Scripting.Dictionary"): d.CompareMode = vbTextCompare
    Dim ws As Worksheet
    On Error Resume Next: Set ws = ThisWorkbook.Sheets(Module_Config.CfgTextOr("SHEET_Personnel", "Personnel")): On Error GoTo 0
...
Private Function ExtractHoursFromCode(ByVal code As String, ByRef hStart As Double, ByRef hEnd As Double) As Boolean
    ...
    specialValue = Module_Config.CfgTextOr(specialKey, "")
