...
Public Sub GenererDatesEtJoursPourTousLesMois()
    ...
    ' Lire configuration
    '========================
    FIRST_DAY_COL = CLng(CfgValueOr("PLN_FirstDayCol", 0))          ' ex: 3 (C)
    LAST_DAY_COL = CLng(CfgValueOr("PLN_LastDayCol", 0))            ' ex: 33 (AG)
    ROW_JOUR_SEMAINE = CLng(CfgValueOr("PLN_Row_DayNames", 0))      ' ex: 3
    ROW_NUMERO_JOUR = CLng(CfgValueOr("PLN_Row_DayNumbers", 0))     ' ex: 4
    
    annee = CLng(CfgValueOr("CFG_Year", 0))                         ' ex: 2026
    
    keepHeaderRows = CfgTextOr("VIEW_HeaderRows_Keep", "")    ' ex: "3:4" ou "1:4"
    yearCellAddress = CfgTextOr("VIEW_YearCell", "")          ' ex: "B1" (fallback si vide)
    If Len(yearCellAddress) = 0 Then yearCellAddress = "B1"
    
    localeKey = CfgTextOr("CFG_Locale", "")                   ' ex: "fr-BE"
    ...
    colorWeekendOrHoliday = ParseRgbTextToLong(CfgTextOr("PAL_Color_WeekendOrHoliday", ""), RGB(255, 0, 0))
