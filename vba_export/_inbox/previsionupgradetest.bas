...
Attribute VB_Name = "previsionupgradetest"
Option Explicit
...
Const SUGG_C15_GRP As Long = 5 ' Array("C 15", "C 15 di")
...
Function ContientCodeDuGroupe(planningArr As Variant, rempContextArr As Variant, jourCol As Long, groupCodes As Variant) As Boolean
    ...
    If IsEmpty(groupCodes) Then Exit Function
    If Not IsArray(groupCodes) Then Exit Function
    If UBound(groupCodes) < LBound(groupCodes) Then Exit Function
...
Sub MettreAJourCompteursMAS(codePlace As String, ByRef actualMatin As Long, ByRef actualPM As Long, ByRef actualSoir As Long, ByVal ligneEnCours As Long, ByRef presence7_8h_compteur As Long)
    Select Case codePlace
        ...
        Case "C 20", "C 20 E"
            ...
            actualSoir = actualSoir + 1
        Case "C 19", "C 19 di"
            actualMatin = actualMatin + 1 ' Selon la logique originale, C19 contribue au Matin
            actualSoir = actualSoir + 1
            presence7_8h_compteur = presence7_8h_compteur + 1
        Case "12:30 16:30"
            actualPM = actualPM + 1
        Case "8:30 12:45 16:30 20:15"
            If ligneEnCours = LIGNE_DEBUT_PLANNING_PERSONNEL Then ' Normalement la ligne de "Hermann"
                actualPM = actualPM + 1
                actualSoir = actualSoir + 1
            End If
    End Select
...
Sub TraiterUneFeuilleDeMois(ws As Worksheet, _
    ...
    ' Charger les effectifs initiaux des lignes 60-62 dans des tableaux une seule fois
    ...
    For col = 1 To nbJours ' Itérer à travers les colonnes (jours)
        ...
        For l = 1 To NB_REMPLACEMENT_JOUR_LIGNES
            If Trim(CStr(rempJourArr(l, col))) = "" Then
                ...
                ' --- SINON ET manqueSoir > 0 pour AUTRES JOURS ---
