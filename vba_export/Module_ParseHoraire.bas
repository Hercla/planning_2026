...
Public Function HeureEnDecimal(ByVal heureStr As String) As Double
    ' Convertit "HH:MM" ou "HH" en decimal
    Dim parts() As String
...
Public Sub CalculerPresencesSpecifiques(hd As Double, hf As Double, hd2 As Double, hf2 As Double, _
    ...
    End If
End Sub
Public Function EstHoraireSpecial(h1 As Double, f1 As Double, h2 As Double, f2 As Double, defCode As String) _
      As Boolean
      EstHoraireSpecial = False
      If defCode = "" Then Exit Function
      ...
      If Abs(h1 - c1) < TOL And Abs(f1 - c2) < TOL And Abs(h2 - c3) < TOL And Abs(f2 - c4) < TOL Then
