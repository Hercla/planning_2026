...
Public Sub VIEW_Apply_ByScope()
    Dim scope As String
    scope = UCase$(Trim$(Module_Config.CfgTextOr("VIEW_ApplyScope", ""))) ' ACTIVE / ALL
...
Public Sub VIEW_ApplyToSheet(ByVal ws As Worksheet)
    ...
    If modeLocal = "JOUR" Or modeLocal = "NUIT" Then
        teamMode = modeLocal
    Else
        teamMode = UCase$(Trim$(Module_Config.CfgTextOr("TEAM_MODE", ""))) ' fallback global
...
Public Sub VIEW_ToggleMode_ActiveSheet()
    ...
    If cur <> "JOUR" And cur <> "NUIT" Then
        cur = UCase$(Trim$(Module_Config.CfgTextOr("TEAM_MODE", "")))
...
Private Sub VIEW_ApplyToSheet_WithMode(ByVal ws As Worksheet, ByVal teamMode As String)
    ...
    If teamMode = "NUIT" Then
        ApplyHideBlocks ws, Module_Config.CfgTextOr("VIEW_Nuit_HideBlocks", "")
    Else
        ApplyHideBlocks ws, Module_Config.CfgTextOr("VIEW_Jour_HideBlocks", "")
    End If
...
Private Sub ApplyZoom(ByVal ws As Worksheet)
    Dim z As Long
    z = Module_Config.CfgLong("VIEW_Zoom")
...
Private Sub ApplyHideMenuCols(ByVal ws As Worksheet)
    Dim cols As String
    cols = Trim$(Module_Config.CfgTextOr("VIEW_MenuCols", "")) ' ex: "AH:AO"
...
Private Sub ApplyHideColumnB(ByVal ws As Worksheet)
    ...
    If Module_Config.CfgBool("VIEW_HideColumnB") Then
...
Private Sub ApplyAutoHideNames(ByVal ws As Worksheet)
    ...
    colLetter = Trim$(Module_Config.CfgTextOr("VIEW_NameCol_A", "")) ' ex: "A"
    If Len(colLetter) = 0 Then colLetter = "A"
    
    firstRow = Module_Config.CfgLong("VIEW_AutoHide_FirstRow")
    lastRow = Module_Config.CfgLong("VIEW_AutoHide_LastRow")
...
Private Function ShouldApplyViewToSheet(ByVal ws As Worksheet) As Boolean
    ...
    cfgSheetName = Module_Config.CfgTextOr("SHEET_FeuilConfig", "")
